# This code is meant for incremental load. The code will match the hive table rows with 
#Postgres table based on a unique key and append any additonal new record created in Postgres to Hive
...
# Hive table
hive_database = "weather_forecast"
hive_table_name = "emp"
hive_table_full_name = "{}.{}".format(hive_database, hive_table_name)

# Find the max(id) in the Hive table
hive_max_id = spark.sql("SELECT MAX(emp_id) as max_id FROM {}".format(hive_table_full_name)).collect()[0]['max_id']
...
postgres_query = "(SELECT * FROM emp WHERE emp_id > {}) AS E1".format(hive_max_id)
...
if df_postgres.count() > 0:
    ...
    df_postgres.write.mode("append").saveAsTable(hive_table_full_name)
